...
export default function Costos() {
  ...
  // Business Rule: Check if a costo is editable by the current user
    ...
    <div className="space-y-6 pb-20 md:pb-0">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-3xl font-black text-foreground tracking-tight">Costos</h1>
            {isSysAdmin && (
        ...
        <div className="bg-card border border-primary/20 p-4 rounded-3xl shadow-sm space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-bold text-foreground">Ver todos los costos (incluyendo empresas)</span>
            </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center p-12 bg-card rounded-3xl border border-border border-dashed animate-pulse">
          ...
          <p className="text-muted-foreground font-bold tracking-widest uppercase text-xs">Cargando costos...</p>
        </div>
      ) : (
        <>
          {/* Mobile View: Cards */}
          <div className="grid grid-cols-1 gap-4 sm:hidden">
            {filteredCostos?.map(costo => {
              ...
              return (
                <div key={costo.id} className={`bg-card border border-border p-5 rounded-3xl shadow-sm flex flex-col gap-4 transition-opacity ${!costo.activo ? 'opacity-70' : ''}`}>
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <h3 className="font-black text-foreground text-lg leading-tight">{costo.nombre}</h3>
                        {costo.idEmpresa === null && <span title="Costo Global"><Globe size={14} className="text-blue-500" /></span>}
                        {!costo.activo && <span className="text-[10px] font-black uppercase text-destructive dark:text-destructive-foreground bg-destructive/30 px-1.5 py-0.5 rounded-md">Inactivo</span>}
                      </div>
                      {costo.descripcion && <p className="text-xs text-muted-foreground line-clamp-2">{costo.descripcion}</p>}
                    </div>
                    <div className="flex gap-2">
                      {editable ? (
                        <>
                          <button onClick={() => handleOpenModal(costo)} className="p-2 rounded-xl bg-accent text-primary hover:bg-primary/10 transition-colors cursor-pointer" disabled={updatingIds.has(costo.id)}>
                            <Pencil size={16} />
                          </button>
                          <button
                            onClick={() => handleToggleActivo(costo)}
                            className={`p-2 rounded-xl bg-accent transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed ${costo.activo ? 'text-emerald-500 hover:bg-emerald-500/10' : 'text-muted-foreground hover:bg-muted'}`}
                        ...
                        <div className="p-2 rounded-xl bg-muted text-muted-foreground" title="Este costo es global y no puede ser editado">
                          <Lock size={16} />
                        </div>
                      )}
                    </div>
                  ...
                  {(isSysAdmin || isAsesor) && costo.idEmpresa !== null && (
                    <div className="pt-2 border-t border-border/50 text-[10px] font-bold text-muted-foreground flex items-center gap-1">
                      <Package size={10} />
                      Empresa: {empresas.find(e => e.id === costo.idEmpresa)?.nombre || `ID: ${costo.idEmpresa}`}
                    </div>
                  )}
                </div>
              )
          ...
          <div className="hidden sm:block overflow-hidden bg-card border border-border rounded-3xl shadow-sm">
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-border/50">
                  {filteredCostos?.map(costo => {
                    const editable = isEditable(costo)
                    return (
                      <tr key={costo.id} className={`group transition-colors ${costo.activo ? 'hover:bg-accent/20' : 'bg-destructive/5 opacity-70'}`}>
                        <td className="px-6 py-4">
                          <span className={`font-bold ${costo.activo ? 'text-foreground' : 'text-muted-foreground'}`}>{costo.nombre}</span>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          {costo.idEmpresa === null ? (
                            ...
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-orange-100 dark:bg-orange-900/30 text-orange-800 dark:text-orange-300 text-[10px] font-black uppercase tracking-wider">
                              <Package size={12} />
                              {(isSysAdmin || isAsesor) ? (
                                `${empresas.find(e => e.id === costo.idEmpresa)?.nombre || 'Empresa'} (ID: ${costo.idEmpresa})`
                              ) : 'Mi Empresa'}
                            </span>
                          )}
                        </td>
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${costo.activo ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-400' : 'bg-destructive/30 text-destructive dark:text-destructive-foreground'}`}>
                          ...
                          <div className="flex justify-end gap-2">
                            {editable ? (
                              <>
                                ...
                                <button
                                  onClick={() => handleToggleActivo(costo)}
                                  className={`p-2 rounded-xl bg-accent transition-all shadow-xs group-hover:scale-110 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed ${costo.activo ? 'text-emerald-500 hover:bg-emerald-500 hover:text-white' : 'text-muted-foreground hover:bg-destructive hover:text-white'}`}
                                  title={costo.activo ? 'Desactivar' : 'Activar'}
                                  disabled={updatingIds.has(costo.id)}
                                >
                                  {updatingIds.has(costo.id) ? (
            ...
            {filteredCostos?.length === 0 && (
              <div className="p-12 text-center">
                <DollarSign className="size-12 text-muted-foreground/20 mx-auto mb-4" />
                <p className="text-muted-foreground font-medium">No se encontraron costos.</p>
              </div>
            )}
          </div>
        </>
