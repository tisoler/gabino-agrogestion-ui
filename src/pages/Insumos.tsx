...
export default function Insumos() {
  ...
  // Business Rule: Check if an insumo is editable by the current user
    ...
    <div className="space-y-6 pb-20 md:pb-0">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-3xl font-black text-foreground tracking-tight">Insumos</h1>
            {isSysAdmin && (
        ...
        {canWrite && (
          <button
            ...
            className="flex items-center gap-2 px-5 py-3 bg-primary text-primary-foreground rounded-2xl font-bold shadow-lg shadow-primary/20 hover:shadow-xl hover:shadow-primary/30 hover:-translate-y-0.5 active:translate-y-0 transition-all w-full sm:w-auto justify-center"
          >
            <Plus size={20} />
        ...
        <div className="bg-card border border-primary/20 p-4 rounded-3xl shadow-sm space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-bold text-foreground">Ver todos los insumos (incluyendo empresas)</span>
            </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center p-12 bg-card rounded-3xl border border-border border-dashed animate-pulse">
          ...
          <p className="text-muted-foreground font-bold tracking-widest uppercase text-xs">Cargando insumos...</p>
        </div>
      ) : (
        <>
          {/* Mobile View: Cards */}
          <div className="grid grid-cols-1 gap-4 sm:hidden">
            {filteredInsumos?.map(insumo => {
              ...
              return (
                <div key={insumo.id} className={`bg-card border border-border p-5 rounded-3xl shadow-sm flex flex-col gap-4 transition-opacity ${!insumo.activo ? 'opacity-70' : ''}`}>
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <h3 className="font-black text-foreground text-lg leading-tight">{insumo.nombre}</h3>
                        {insumo.idEmpresa === null && <span title="Insumo Global"><Globe size={14} className="text-blue-500" /></span>}
                        {!insumo.activo && <span className="text-[10px] font-black uppercase text-destructive dark:text-destructive-foreground bg-destructive/30 px-1.5 py-0.5 rounded-md">Inactivo</span>}
                      </div>
                      {insumo.descripcion && <p className="text-xs text-muted-foreground line-clamp-2">{insumo.descripcion}</p>}
                    </div>
                    <div className="flex gap-2">
                      {editable ? (
                        <>
                          <button onClick={() => handleOpenModal(insumo)} className="p-2 rounded-xl bg-accent text-primary hover:bg-primary/10 transition-colors cursor-pointer" disabled={updatingIds.has(insumo.id)}>
                            <Pencil size={16} />
                          </button>
                          <button
                            onClick={() => handleToggleActivo(insumo)}
                            className={`p-2 rounded-xl bg-accent transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed ${insumo.activo ? 'text-emerald-500 hover:bg-emerald-500/10' : 'text-muted-foreground hover:bg-muted'}`}
                        ...
                        <div className="p-2 rounded-xl bg-muted text-muted-foreground" title="Este insumo es global y no puede ser editado">
                          <Lock size={16} />
                        </div>
                      )}
                    </div>
          ...
          <div className="hidden sm:block overflow-hidden bg-card border border-border rounded-3xl shadow-sm">
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-border/50">
                  {filteredInsumos?.map(insumo => {
                    const editable = isEditable(insumo)
                    return (
                      <tr key={insumo.id} className={`group transition-colors ${insumo.activo ? 'hover:bg-accent/20' : 'bg-destructive/5 opacity-70'}`}>
                        <td className="px-6 py-4">
                          <span className={`font-bold ${insumo.activo ? 'text-foreground' : 'text-muted-foreground'}`}>{insumo.nombre}</span>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          {insumo.idEmpresa === null ? (
                            ...
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-orange-100 dark:bg-orange-900/30 text-orange-800 dark:text-orange-300 text-[10px] font-black uppercase tracking-wider">
                              <Package size={12} />
                              {(isSysAdmin || isAsesor) ? (
                                `${empresas.find(e => e.id === insumo.idEmpresa)?.nombre || 'Empresa'} (ID: ${insumo.idEmpresa})`
                              ) : 'Mi Empresa'}
                            </span>
                          )}
                        </td>
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${insumo.activo ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-400' : 'bg-destructive/30 text-destructive dark:text-destructive-foreground'}`}>
                          ...
                          <div className="flex justify-end gap-2">
                            {editable ? (
                              <>
                                ...
                                <button
                                  onClick={() => handleToggleActivo(insumo)}
                                  className={`p-2 rounded-xl bg-accent transition-all shadow-xs group-hover:scale-110 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed ${insumo.activo ? 'text-emerald-500 hover:bg-emerald-500 hover:text-white' : 'text-muted-foreground hover:bg-destructive hover:text-white'}`}
                                  title={insumo.activo ? 'Desactivar' : 'Activar'}
                                  disabled={updatingIds.has(insumo.id)}
                                >
                                  {updatingIds.has(insumo.id) ? (
            ...
            {filteredInsumos?.length === 0 && (
              <div className="p-12 text-center">
                ...
                <p className="text-muted-foreground font-medium">No se encontraron insumos.</p>
              </div>
            )}
          </div>
        </>
          ...
          <div className="relative w-full max-w-lg bg-card border border-border shadow-2xl rounded-3xl overflow-hidden animate-in fade-in zoom-in duration-300">
            ...
            <form className="p-6 space-y-4" onSubmit={handleSubmit}>
              ...
              <div className="flex gap-3 pt-6">
                ...
                <button
                  type="submit"
                  className="flex-1 px-5 py-3 bg-primary text-primary-foreground rounded-xl font-bold shadow-lg shadow-primary/20 hover:shadow-xl transition-all hover:-translate-y-0.5"
                >
